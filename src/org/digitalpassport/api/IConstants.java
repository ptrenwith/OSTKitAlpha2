
package org.digitalpassport.api;

/**
 *
 * @author Philip M. Trenwith
 */
public interface IConstants 
{
  public static final String API_KEY = "15f7ecf01a98f63ad3c6";
  
  public static final String m_sURL = "https://playgroundapi.ost.com";
  
  public static final String g_sPATH_USERS_LIST             = "/users/list";
  public static final String g_sPATH_USERS_EDIT             = "/users/edit";
  public static final String g_sPATH_USERS_CREATE           = "/users/create";
  public static final String g_sPATH_USERS_AIRDROP_DROP     = "/users/airdrop/drop";
  public static final String g_sPATH_USERS_AIRDROP_STATUS      = "/users/airdrop/status";
  public static final String g_sPATH_TRANSACTION_TYPES_STATUS  = "/transaction-types/status";
  public static final String g_sPATH_TRANSACTION_TYPES_CREATE  = "/transaction-types/create";
  public static final String g_sPATH_TRANSACTION_TYPES_EDIT    = "/transaction-types/edit";
  public static final String g_sPATH_TRANSACTION_TYPES_LIST    = "/transaction-types/list";
  public static final String g_sPATH_TRANSACTION_TYPES_EXECUTE = "/transaction-types/execute";
  
  public static final String g_sPARAM_SUCCESS               = "success";
  public static final String g_sPARAM_DATA                  = "data";
  public static final String g_sPARAM_ERROR                 = "err";
  public static final String g_sPARAM_DISPLAY_TEXT          = "display_text";
  public static final String g_sPARAM_DISPLAY_HEADING       = "display_heading";
  public static final String g_sPARAM_AIRDROP_UUID          = "airdrop_uuid";
  public static final String g_sPARAM_CURRENT_STATUS        = "current_status";
  public static final String g_sPARAM_STEPS_COMPLETED       = "steps_complete";
  
  public static final String g_sPARAM_ID                    = "id";
  public static final String g_sPARAM_CLIENT_TRANSACTION_ID = "client_transaction_id";
  public static final String g_sPARAM_CLIENT_ID             = "client_id";
  public static final String g_sPARAM_NAME                  = "name";
  public static final String g_sPARAM_KIND                  = "kind";
  public static final String g_sPARAM_CURRENCY_TYPE         = "currency_type";
  public static final String g_sPARAM_CURRENCY_VALUE        = "currency_value";
  public static final String g_sPARAM_COMMISSION_PERCENT    = "commission_percent";
  public static final String g_sPARAM_STATUS                = "status";
  public static final String g_sPARAM_TIMESTAMP             = "uts";
  public static final String g_sPARAM_TRANSACTIONS          = "transactions";
  public static final String g_sPARAM_TRANSACTION_UUIDS     = "transaction_uuids";
  public static final String g_sPARAM_TRANSACTION_TYPES     = "transaction_types";
  
  public static final String g_sPARAM_TRANSACTION_UUID      = "transaction_uuid";
  public static final String g_sPARAM_TRANSACTION_HASH      = "transaction_hash";
  public static final String g_sPARAM_TRANSACTION_FROM_UUID = "from_uuid";
  public static final String g_sPARAM_TRANSACTION_TO_UUID   = "to_uuid";
  public static final String g_sPARAM_TRANSACTION_KIND      = "transaction_kind";
  
  public static final String g_sPARAM_FROM_USER_ID          = "from_user_id";
  public static final String g_sPARAM_TO_USER_ID            = "to_user_id";
  public static final String g_sPARAM_TRANSACTION_TYPE_ID   = "transaction_type_id";
  public static final String g_sPARAM_CLIENT_TOKEN_ID       = "client_token_id";
  public static final String g_sPARAM_GAS_PRICE             = "gas_price";
  public static final String g_sPARAM_GAS_USED              = "gas_used";
  public static final String g_sPARAM_TRANSACTION_TIMESTAMP = "transaction_timestamp";
  public static final String g_sPARAM_TRANSACTION_FEE       = "transaction_fee";
  public static final String g_sPARAM_BLOCK_NUMBER          = "block_number";
  public static final String g_sPARAM_BT_TRANSFER_VALUE     = "bt_transfer_value";
  public static final String g_sPARAM_BT_COMMISSION_AMOUNT  = "bt_commission_amount";
  
  public static final String g_sPARAM_PRICE_POINTS          = "price_points";
  public static final String g_sPARAM_USD                   = "USD";
  public static final String g_sPARAM_OST                   = "OST";
  
  public static final String g_sPARAM_SYMBOL                = "symbol";
  public static final String g_sPARAM_SYMBOL_ICON           = "symbol_icon";
  public static final String g_sPARAM_CONV_FACTOR           = "conversion_factor";
  public static final String g_sPARAM_TOKEN_ERC20_ADDR      = "token_erc20_address";
  public static final String g_sPARAM_AIRDROP_CONTRACT_ADDR = "airdrop_contract_addr";
  public static final String g_sPARAM_SIMPLE_STAKE_CONTRACT_ADDR = "simple_stake_contract_addr";
  public static final String g_sPARAM_CLIENT_TOKENS         = "client_tokens";
  
  public static final String g_sPARAM_API_KEY               = "api_key";
  public static final String g_sPARAM_REQ_TIMESTAMP         = "request_timestamp";
  public static final String g_sPARAM_SIGNATURE             = "signature";
  public static final String g_sPARAM_PAGE_NO               = "page_no";
  public static final String g_sPARAM_FILTER                = "filter";
  public static final String g_sPARAM_ORDER_BY              = "order_by";
  public static final String g_sPARAM_ORDER                 = "order";
  
  public static final String g_sPARAM_UUID                  = "uuid";
  
  public static final String g_sPARAM_CODE                  = "code";
  public static final String g_sPARAM_MSG                   = "msg";
  public static final String g_sPARAM_ERROR_DATA            = "error_data";
  
  public static final String g_sPARAM_TOKENS_AIRDROPPED     = "total_airdropped_tokens";
  public static final String g_sPARAM_TOKEN_BALANCE         = "token_balance";
  public static final String g_sPARAM_RESULT_TYPE           = "result_type";
  public static final String g_sPARAM_ECONOMY_USERS         = "economy_users";
  public static final String g_sPARAM_META                  = "meta";
  public static final String g_sPARAM_NEXT_PAGE             = "next_page_payload";
  
  public static final String g_sPARAM_AMOUNT                = "amount";
  public static final String g_sPARAM_LIST_TYPE             = "list_type";
  
  public static final String g_sPARAM_AIRDROP_USERS_IDENTIFIED    = "users_identified";
  public static final String g_sPARAM_AIRDROP_TOKENS_TRANSFERED   = "tokens_transfered";
  public static final String g_sPARAM_AIRDROP_CONTRACT_APPORVED   = "contract_approved";
  public static final String g_sPARAM_AIRDROP_ALOC_DONE           = "allocation_done";
}
